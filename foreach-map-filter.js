...
strSplit.forEach(function(value){
    ...
    if(vowels.indexOf(lowerCased) !== -1){
        if (vowelsCount[lowerCased]){
            vowelsCount[lowerCased]++;
        }
        else
        {
            vowelsCount[lowerCased] = 1;
        }
    }
});
return vowelsCount;
...
/*
Write a function called extractFullName which accepts an array of objects and returns a new array with the value of the key with a name of "first" and the 
...
let newArr= arr.filter(x => x%2 !==0).map(function(x){
    return x*2
})
